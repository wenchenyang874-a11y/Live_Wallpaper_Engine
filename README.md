...
- GIF 由 WIC 按帧解码和合成；视频通过 Media Foundation/Media Engine 循环播放，并优先使用系统硬件解码能力。
...
在“我的壁纸”中点击“导入壁纸”，或把文件直接拖入窗口，即可加入本地库；选中后可应用或导出分享包。当前使用一个覆盖窗口跨越整个虚拟桌面并执行居中填充；退出程序不会改动或重设 Windows 原壁纸。逐显示器设置和更多缩放模式仍待实现。
